...
import csv
dataPath = Path('/mnt/cnets/datasets/bluesky/firehose_archives')
...
for date in tqdm(dateRange):
    # filenames are like: 2024-07-01.json.gz
    filename = dataPath / f"{date}.json.gz"
...
uri2Index = {uri: i for i, uri in enumerate(df["uri"])}
...
dfOutputWithReposts.to_feather(outputPath / f"{datasetName}.feather")


# rename columns
# post_id -> tweet_id
# post_type -> tweet_type  # post->tweet,repost->retweet,reply
# linked_post -> linked_tweet
# linked_post_user_id -> linked_tweet_user_id

renames = {
    "post_id": "tweet_id",
    "post_type": "tweet_type",
    "linked_post": "linked_tweet",
    "linked_post_user_id": "linked_tweet_user_id"
}
dfOutputWithRepostsTransformed = dfOutputWithReposts.rename(columns=renames)
# change post->tweet, reply->reply, repost->retweet
dfOutputWithRepostsTransformed["tweet_type"] = dfOutputWithRepostsTransformed["tweet_type"].map({"post": "tweet", "reply": "reply", "repost": "retweet"})
dfOutputWithRepostsTransformed.to_csv(outputPath / f"{datasetName}_tweetsFormat.csv",index=False,escapechar='\\', encoding='utf-8',quoting=csv.QUOTE_NONNUMERIC)

# [x] user_id
# [x] post_id
# [x] post_type # post,repost,reply
# [x] creation_date
# [x] text
# [x] linked_post
# [x] linked_post_user_id
# [x] root_post
# [x] root_post_user_id
# [x] hashtags
# [x] mentioned_users
# [x] urls 
# [x] data_langs: split and join by 
